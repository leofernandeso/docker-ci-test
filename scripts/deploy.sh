...
# Tagging
docker tag ${APP_NAME} gcr.io/${PROJECT_ID}/${APP_NAME}:v4

# Pushing to container registry
gcloud docker -- push gcr.io/${PROJECT_ID}/${APP_NAME}:v4

echo "KubeCTL deploying..."
# Updating existing deploy
kubectl set image deployment/${DEPLOYMENT_NAME} ${CONTAINER_NAME}=gcr.io/${PROJECT_ID}/${APP_NAME}:v4